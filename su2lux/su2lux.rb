...
module SU2LUX
	...
	def SU2LUX.export
		...
		# Export section planes if enabled
		if lrs.use_section_planes
			puts 'exporting section planes'
			le.export_section_planes(lxs_file)
		end
		...
		# Write section plane configuration for LuxCore
		if lrs.use_section_planes
			section_config = le.get_section_plane_config
			if section_config
				cfg_file_path = file_fullname + ".cfg"
				puts "Writing section plane config to: #{cfg_file_path}"
				cfg_file = File.new(cfg_file_path, "w")
				cfg_file.puts "# LuxCoreRender Clipping Plane Configuration"
				cfg_file.puts "# Generated by SU2LUX"
				cfg_file.puts "scene.camera.clippingplane.enable = 1"
				cfg_file.puts "scene.camera.clippingplane.center = #{section_config['center_x']} #{section_config['center_y']} #{section_config['center_z']}"
				cfg_file.puts "scene.camera.clippingplane.normal = #{section_config['normal_x']} #{section_config['normal_y']} #{section_config['normal_z']}"
				cfg_file.close
			end
		end
...
#####################################################################
    ...
    ##
    # Check if LuxCoreRender path is valid
    ##
    def SU2LUX.luxrender_path_valid?(luxrender_path)
        return false if luxrender_path.nil?
        basename = File.basename(luxrender_path).upcase
        # Check for LuxCoreRender executables (luxcoreui) or legacy LuxRender
        (basename.include?("LUXCOREUI") || basename.include?("LUXCORE") || basename.include?("LUSUVIEWER") || basename.include?("LUXRENDER"))
    end #END luxrender_path_valid?
  
	##
	# Launch LuxCoreRender with the exported scene
	##
	def SU2LUX.launch_luxrender
		...
		export_path = File.join(File.dirname(lrs.export_file_path), SU2LUX.sanitize_path(File.basename(lrs.export_file_path)))
		...
		if (ENV['OS'] =~ /windows/i)
			# Windows: Launch LuxCoreUI with scene file
			command_line = "start \"su2lux\" \"#{lrs.export_luxrender_path}\" \"#{export_path}\""
			puts command_line
			system(command_line)
        else
            # macOS: Launch LuxCoreUI
			luxcore_path = lrs.export_luxrender_path
			# If path is a .app bundle, get the actual executable
			if luxcore_path.end_with?(".app")
				luxcore_path = File.join(luxcore_path, "Contents", "MacOS", "luxcoreui")
			end
			Thread.new do
				system("\"#{luxcore_path}\" \"#{export_path}\"")
			end
		end
	...
	##
    # Create network settings editor
    ##
    def SU2LUX.create_network_settings_editor(scene_id, lrs)
        @networkedit_hash = {} unless @networkedit_hash
        @networkedit_hash[scene_id] = LuxrenderNetworkSettingsEditor.new(scene_id, lrs)
        return @networkedit_hash[scene_id]
    end
    ...
    end # END show_render_settings_editor
	
	##
	# Show network settings editor
	##
	def SU2LUX.show_network_settings_editor(scene_id)
        puts "running show network settings editor"
		@networkedit_hash = {} unless @networkedit_hash
		
		# Create editor if it doesn't exist
		if not @networkedit_hash[scene_id]
			lrs = SU2LUX.get_lrs(scene_id)
			@networkedit_hash[scene_id] = LuxrenderNetworkSettingsEditor.new(scene_id, lrs)
		end
		
        if @networkedit_hash[scene_id].visible?
            @networkedit_hash[scene_id].close
        else
            @networkedit_hash[scene_id].show
        end
		
    end # END show_network_settings_editor
...
if( not file_loaded?(__FILE__))
	...
	load File.join(SU2LUX::PLUGIN_FOLDER, "LuxrenderNetworkSettingsEditor.rb")
